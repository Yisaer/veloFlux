...
These scripts support perf validation for multi-`flow_instance` CPU isolation with
thread-level in-process extra flow instances.

Extra flow instances run in-process with dedicated Tokio runtimes and bind with `cgroup.threads`.
...
Update the cgroup paths in the config file to match your `--base-cg`.
...
The cleanup script is topology-aware enough to work for the thread_level perf mode.
