...
use crate::expr::custom_func::helpers::{
    any_null, nth_array_or_null, nth_i64_or_null, nth_map_or_null, nth_string_or_null,
    validate_arity, validate_at_least_arity, validate_one_array_or_null, validate_two_arrays_or_null,
    value_sort_key, value_to_array, value_to_i64, value_to_map, value_to_string,
    value_to_string_lossy, array_distinct_values,
};
use std::sync::Arc;
...
impl CustomFunc for ArrayPositionFunc {
    fn validate_row(&self, args: &[Value]) -> Result<(), EvalError> {
        ...
        if !matches!(args.get(0), Some(Value::Null) | Some(Value::List(_))) {
        ...
        match &args[0] {
            Value::List(array) => {
                ...
                if index >= 0 {
                    ...
                    Ok(array[(len + index) as usize].clone())
                }
            }
            Value::Struct(map) => {
...
impl CustomFunc for ArrayRemoveFunc {
    ...
    fn eval_row(&self, args: &[Value]) -> Result<Value, EvalError> {
        ...
        Ok(Value::List(
            array.into_iter().filter(|x| x != &args[1]).collect(),
        ))
    }
...
impl CustomFunc for RepeatFunc {
    ...
    fn eval_row(&self, args: &[Value]) -> Result<Value, EvalError> {
        ...
        Ok(Value::List(vec![args[0].clone(); count as usize]))
    }
...
impl CustomFunc for ArrayDistinctFunc {
    ...
    fn eval_row(&self, args: &[Value]) -> Result<Value, EvalError> {
        ...
        Ok(Value::List(array_distinct_values(array)))
    }
...
fn test_array_functions_correctness() {
    ...
    assert_int(cardinality.eval_row(&[a(vec![i(1), i(2), i(3)])]).unwrap(), 3);
    ...
    assert_eq!(
        element_at.eval_row(&[a(vec![i(10), i(20), i(30)]), i(1)]).unwrap(),
        i(20)
    );
    assert_eq!(
        element_at.eval_row(&[a(vec![i(10), i(20), i(30)]), i(-1)]).unwrap(),
    ...
    assert_null(array_max.eval_row(&[a(vec![Value::Null, Value::Null])]).unwrap());
    ...
    assert_null(array_min.eval_row(&[a(vec![Value::Null, Value::Null])]).unwrap());
    ...
    assert_null(array_except.eval_row(&[Value::Null, a(vec![i(1)])]).unwrap());
    assert_array(
        array_except
    ...
    assert_array(repeat.eval_row(&[s("x"), i(3)]).unwrap(), vec![s("x"), s("x"), s("x")]);
    ...
    assert_array(
        array_flatten
            .eval_row(&[a(vec![a(vec![i(1)]), i(2), a(vec![i(3), i(4)])])])
            .unwrap(),
        vec![i(1), i(2), i(3), i(4)],
    );
    ...
    match shuffled {
        Value::List(v) => {
            assert_eq!(v.len(), 4);
            assert!(v.contains(&i(1)));
            assert!(v.contains(&i(2)));
            assert!(v.contains(&i(3)));
            assert!(v.contains(&i(4)));
        }
    ...
    assert_array(
        array_sort
            .eval_row(&[a(vec![i(3), i(2), s("b"), s("a")])])
            .unwrap(),
        vec![i(2), i(3), s("a"), s("b")],
    );
...
fn test_array_functions_arity_and_error_cases() {
    ...
    assert!(ArrayPositionFunc.eval_row(&[a(vec![i(1)]), i(1), i(2)]).is_err());
    ...
    assert!(ArrayContainsFunc.eval_row(&[a(vec![i(1)]), i(1), i(2)]).is_err());
    ...
    assert!(array_map.eval_row(&[s("not_exist"), a(vec![s("a")])]).is_err());
    assert!(array_map.eval_row(&[s("concat"), a(vec![s("a")])]).is_err());

    assert!(ArrayJoinFunc.eval_row(&[a(vec![i(1)])]).is_err());
    assert!(ArrayJoinFunc.eval_row(&[a(vec![i(1)]), s(","), s("x"), s("y")]).is_err());
