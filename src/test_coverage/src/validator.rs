...
fn validate_interaction_features(
    ...
    if interaction.features.len() < 2 {
        errors.push(ValidationError {
            location: location.clone(),
            message: format!(
                "interaction `{}` must reference at least two features",
                interaction.id
            ),
        });
    }
...
mod tests {
    ...
    #[test]
    fn reports_retired_interaction_with_too_few_features() {
        let mut features = BTreeMap::new();
        features.insert(
            "pipeline.runtime.eventtime".to_string(),
            FeatureDefinition {
        ...
        let mut interaction_entries = BTreeMap::new();
        interaction_entries.insert(
            "runtime.retired_single".to_string(),
            InteractionDefinition {
                id: "runtime.retired_single".to_string(),
                domain: "runtime".to_string(),
                title: "Retired single-feature interaction".to_string(),
                summary: "Summary".to_string(),
                features: vec!["pipeline.runtime.eventtime".to_string()],
                status: "retired".to_string(),
                source_file: PathBuf::from("runtime.yaml"),
            },
        );
