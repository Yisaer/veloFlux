...
impl ServerContext {
    ...
    fn into_parts(self) -> (FlowInstance, StorageManager, String) {
        (self.instance, self.storage, self.manager_addr)
    }
    ...
    F: Future<Output = ()> + Send + 'static,
{
    ...
    let (instance, storage, manager_addr) = ctx.into_parts();
...
pub fn start_profile_server(opts: &ServerOptions) {
    let _ = start_profile_server_runtime(opts);
}
